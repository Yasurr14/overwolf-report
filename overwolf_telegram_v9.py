...
if __name__ == "__main__":
    ...
    combined_latest_day = 0.0
    combined_latest_day_date = ""
    ...
    for app_id in APP_IDS:
        ...
        for row in rows:
            ...
            if date:
                monthly[date[:7]] += net
                combined_monthly[date[:7]] += net
                if date > latest_date:
                    latest_date = date
                    latest_value = net

        if latest_date:
            combined_latest_day_date = latest_date
            combined_latest_day += latest_value

        this_m = monthly.get(this_month, 0.0)
        last_m = monthly.get(last_month, 0.0)

        msg += "\U0001f4b0 Latest day: $" + str(round(latest_value, 2))
        if latest_date:
            msg += " (" + latest_date + ")"
        msg += "\n"
    ...
    msg += "\U0001f4b0 Latest day: $" + str(round(combined_latest_day, 2))
    if combined_latest_day_date:
        msg += " (" + combined_latest_day_date + ")"
    msg += "\n"
